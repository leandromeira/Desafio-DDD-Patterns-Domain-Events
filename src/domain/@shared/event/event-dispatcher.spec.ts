...
import EnviaConsoleLogEnderecoAlteradoHandler from "../../customer/event/handler/envia-console-log-endereco-alterado.handler";
...
describe("Domain events tests", () => {
  ...
  it("should notify all event handlers when a customer is created", () => {
    ...
    const customerCreatedEvent = new CustomerCreatedEvent(
      new Customer("123", "John Dee")
    );

    // Quando o notify for executado o EnviaConsoleLog1Handler.handle() e EnviaConsoleLog2Handler.handle() devem ser chamados
  ...
  it("should notify all event handlers when the address of a customer is modified", () => {
    const eventDispatcher = new EventDispatcher();
    const eventHandler = new EnviaConsoleLogEnderecoAlteradoHandler();
    const spyEventHandler = jest.spyOn(eventHandler, "handle");
    ...
    let customer = new Customer("123", "John Dee")

    expect(customer.Address).toBe(undefined)

    const addressChangedEvent = new AddressChangedEvent(
      customer.changeAddress(new Address("Street 1", 123, "123321-123", "Chicago"))
    );
